...
export default function Settings() {
  ...
  const [localKey, setLocalKey] = useState(
    localStorage.getItem("bikai_api_key") ?? "",
  );
  ...
  async function handleRotate() {
    if (
      !confirm(
        "This will invalidate the current API key. All connected clients will need the new key. Continue?",
      )
    )
      return;
    setBusy(true);
  ...
  function handleForget() {
    ...
    showAlert(
      "info",
      "Key cleared from browser. Re-enter to use protected endpoints.",
    );
  }

  function copy(text: string, which: "browser" | "server" = "browser") {
    const setter = which === "server" ? setCopiedServer : setCopiedBrowser;
    const done = () => {
      setter(true);
    ...
    <div>
      ...
      <div className="card">
        ...
        <p className="text-muted mb-12" style={{ fontSize: 13 }}>
          Enter your API key here. It is stored in your browser's localStorage
          and sent automatically with all requests to protected endpoints.
        </p>
      ...
      {serverKey !== null && (
        <div className="card">
          ...
          <p className="text-muted mb-12" style={{ fontSize: 13 }}>
            This is the key stored in <code>.env</code> on the server. All AI
            inference requests require this as the <code>X-API-Key</code>{" "}
            header.
          </p>
          ...
          <p className="form-hint mt-8">
            Rotating generates a new random key and saves it to{" "}
            <code>.env</code>. The server uses it immediately — no restart
            needed.
          </p>
        </div>
      )}
