...
export default function Dashboard() {
  ...
  const [form, setForm] = useState<StartPayload>({
    model: '', parallel: 4, port: 8000, ctx: 4096, threads: 4,
  })
    ...
    <div>
      ...
      <div className="stat-grid mb-16">
        ...
        <div className="stat-box">
          ...
          <div className="stat-value blue" style={{ fontSize: 16 }}>{status?.uptime ?? '—'}</div>
          <div className="stat-sub">since last start</div>
        </div>
        <div className="stat-box">
          <div className="stat-label">Model</div>
          <div className="stat-value" style={{ fontSize: 13, fontWeight: 600, wordBreak: 'break-all' }}>
            {status?.model_name ?? '—'}
          </div>
        ...
        <div className="stat-box">
          ...
          <div className="stat-value purple">{status?.ctx ? status.ctx.toLocaleString() : '—'}</div>
          <div className="stat-sub">tokens per slot</div>
        </div>
        <div className="stat-box">
          <div className="stat-label">Memory Usage</div>
          <div className="stat-value orange">{status?.mem_mb ? `${status.mem_mb.toLocaleString()} MB` : '—'}</div>
        ...
        <div className="form-grid">
          <div className="form-group" style={{ gridColumn: '1 / -1' }}>
            ...
            <span className="form-hint">
              Select a downloaded model. Download more in the <strong>Models</strong> tab.
            </span>
          </div>
        </div>
        ...
        <div className="form-grid">
          <div className="form-group">
            <label className="form-label">
              <span className="flex items-center gap-8"><Zap size={12} /> Parallel Slots</span>
            </label>
            <input
              className="form-input"
              type="number" min={1} max={32}
              value={form.parallel}
              onChange={e => patch('parallel', Number(e.target.value))}
            />
            <span className="form-hint">Concurrent requests — more = more RAM</span>
          </div>
          <div className="form-group">
            <label className="form-label">
              <span className="flex items-center gap-8"><Activity size={12} /> Port</span>
            </label>
            <input
              className="form-input"
          ...
          <div className="form-group">
            <label className="form-label">
              <span className="flex items-center gap-8"><Database size={12} /> Context Length</span>
            </label>
            <input
              className="form-input"
              type="number" min={512} max={8192} step={512}
              value={form.ctx}
              onChange={e => patch('ctx', Number(e.target.value))}
            />
            <span className="form-hint">Tokens per slot. Max 8192 for Gemma.</span>
          </div>
          <div className="form-group">
            <label className="form-label">
              <span className="flex items-center gap-8"><Cpu size={12} /> CPU Threads</span>
            </label>
            <input
              className="form-input"
        ...
        <div className="btn-group">
          ...
          >
            {busy === 'start'
              ...
              : <><Play size={14} /> {isRunning ? 'Save & Restart' : 'Start Server'}</>
            }
          </button>
          <button
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              <div key={path} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <span className={`badge ${method === 'GET' ? 'info' : 'active'}`}
                  style={{ marginTop: 1, flexShrink: 0 }}>
                  {method}
                </span>
                <div>
                  <span className="mono" style={{ fontSize: 13 }}>
                    http://localhost:{status.port}{path}
                  </span>
